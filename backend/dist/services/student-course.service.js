...
class StudentCourseService {
    ...
    static async getAllCoursesForStudent(studentId) {
        const courses = await (0, database_1.query)(`SELECT
        c.*,
       ...
       ORDER BY c.created_at DESC`, [studentId, studentId]);
        // Parse JSON fields
    ...
    static async getEnrolledCourses(studentId) {
       ...
       ORDER BY e.enrollment_date DESC`, [studentId]);
        // Parse JSON fields
        return courses.map(course => ({
            ...course,
            what_you_will_learn: course.what_you_will_learn
    ...
    static async getCourseById(courseId, studentId) {
        const courses = await (0, database_1.query)(`SELECT
        c.*,
        CASE WHEN e.id IS NOT NULL AND e.status IN ('active', 'completed') THEN TRUE ELSE FALSE END as is_enrolled,
        e.status as enrollment_status,
       ...
       JOIN student_profiles sp ON sp.id = ? AND sp.teacher_id = c.teacher_id
       LEFT JOIN enrollments e ON c.id = e.course_id AND e.student_id = ?
       WHERE c.id = ? AND c.is_published = TRUE`, [studentId, studentId, courseId]);
        if (courses.length === 0) {
            return null;
        }
    ...
    static async enrollInCourse(courseId, studentId) {
        ...
        const courses = await (0, database_1.query)('SELECT * FROM courses WHERE id = ? AND is_published = TRUE', [courseId]);
        if (courses.length === 0) {
            throw new Error('Course not found or not available');
        }
        // Check if already enrolled (active or completed)
        const existingEnrollments = await (0, database_1.query)('SELECT * FROM enrollments WHERE student_id = ? AND course_id = ?', [studentId, courseId]);
        if (existingEnrollments.length > 0) {
            ...
            if (enrollment.status === 'cancelled' || enrollment.status === 'suspended') {
                await (0, database_1.query)(`UPDATE enrollments 
           SET status = 'active', 
           ...
           WHERE id = ?`, [enrollment.id]);
                ...
                await (0, database_1.query)('UPDATE courses SET enrollment_count = enrollment_count + 1 WHERE id = ?', [courseId]);
                // Fetch the updated enrollment
                const updatedEnrollments = await (0, database_1.query)('SELECT * FROM enrollments WHERE id = ?', [enrollment.id]);
                return updatedEnrollments[0];
            }
        }
        // Create new enrollment if none exists
        const result = await (0, database_1.query)(`INSERT INTO enrollments (student_id, course_id, status, progress_percentage)
       VALUES (?, ?, 'active', 0.00)`, [studentId, courseId]);
        const enrollmentId = result.insertId;
        // Update enrollment count
        await (0, database_1.query)('UPDATE courses SET enrollment_count = enrollment_count + 1 WHERE id = ?', [courseId]);
        // Fetch the created enrollment
        const enrollments = await (0, database_1.query)('SELECT * FROM enrollments WHERE id = ?', [enrollmentId]);
        return enrollments[0];
    }
    // Unenroll from a course (cancel enrollment)
    static async unenrollFromCourse(courseId, studentId) {
        ...
        // Update to cancelled
        const result = await (0, database_1.query)(`UPDATE enrollments 
       ...
       WHERE student_id = ? AND course_id = ?`, [studentId, courseId]);
        ...
        if (enrollment.status === 'active' || enrollment.status === 'completed') {
            await (0, database_1.query)('UPDATE courses SET enrollment_count = enrollment_count - 1 WHERE id = ? AND enrollment_count > 0', [courseId]);
        }
    }
    // Update last accessed timestamp
    static async updateLastAccessed(courseId, studentId) {
        await (0, database_1.query)(`UPDATE enrollments 
       SET last_accessed_at = CURRENT_TIMESTAMP 
       WHERE student_id = ? AND course_id = ?`, [studentId, courseId]);
    }
    // Update course progress
    static async updateProgress(courseId, studentId, progressPercentage) {
        await (0, database_1.query)(`UPDATE enrollments 
       ...
       WHERE student_id = ? AND course_id = ?`, [progressPercentage, progressPercentage, progressPercentage, studentId, courseId]);
    }
}
