...
export class WebinarService {
  ...
  static async getWebinarsByTeacher(teacherProfileId: string): Promise<Webinar[]> {
    const webinars = await query<Webinar>(
  ...
  static async getWebinarById(
    ...
    const webinar = await queryOne<Webinar>(
      `SELECT * FROM webinars 
       WHERE id = ? AND teacher_id = ?`,
      [webinarId, teacherProfileId]
    );
    return webinar;
  }
  ...
  static async updateWebinar(
    ...
    return await transaction(async (connection) => {
      ...
      const [existingRows] = await connection.execute(
        'SELECT id FROM webinars WHERE id = ? AND teacher_id = ?',
        [webinarId, teacherProfileId]
      );
      ...
      await connection.execute(
        `UPDATE webinars SET ${updates.join(', ')} WHERE id = ?`,
        values
      );
  ...
  static async getUpcomingWebinars(teacherProfileId: string): Promise<Webinar[]> {
    const webinars = await query<Webinar>(
      `SELECT * FROM webinars 
       WHERE teacher_id = ? 
  ...
  static async getPastWebinars(teacherProfileId: string): Promise<Webinar[]> {
    const webinars = await query<Webinar>(
      `SELECT * FROM webinars 
       WHERE teacher_id = ? 
